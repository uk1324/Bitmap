...
Bitmap::Bitmap(const char* filename) {
	...
	file.read(reinterpret_cast<char*>(&infoHeader), std::min(infoHeaderSize, sizeof(infoHeader)));
...
void Bitmap::applyConvolutionFilter(Matrix matrix)
{
	if (!(matrix.width() & 1 && matrix.height() & 1))
		throw std::exception("Matrix width and height must be an odd number.");
...
void Bitmap::saveToBmp(const char* filename, uint16_t bitCount) const
{
	...
	if (bitCount == 32)
	{
		bmpFileHeader fileHeader{ 0x4D42, sizeof(bmpFileHeader) + sizeof(bmpInfoHeader) + 4 * static_cast<uint32_t>(m_width * m_height), 0, sizeof(bmpFileHeader) + sizeof(bmpInfoHeader) };
	...
	else if (bitCount == 24)
	{
		...
		bmpFileHeader fileHeader{ 0x4D42, sizeof(bmpFileHeader) + sizeof(bmpInfoHeader) + imageSize, 0, sizeof(bmpFileHeader) + sizeof(bmpInfoHeader) };
